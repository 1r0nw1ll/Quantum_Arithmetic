...
Canonical source: codex_on_QA/feature_maps/qa_feature_map_v3.py
"""

QA_COMPLIANCE = "observer=feature_extractor, state_alphabet=qa_invariant_space"
...
def compute_qa_invariants(b: float, e: float) -> Dict[str, float]:
    ...
    inv["a_over_b"] = safe_div(a, b)
    inv["d_over_b"] = safe_div(d, b)
    inv["a_over_d"] = safe_div(a, d)
    inv["d_over_e"] = safe_div(d, e)
    inv["b_over_e"] = safe_div(b, e)
    inv["e_over_b"] = safe_div(e, b)
    ...
    def mod_norm(x: float, m: float) -> float:
        return (x % m) / m
    ...
    n_a = abs(int_floor(a))
    inv["phase24_gcd"] = (math.gcd(n_a, 24) / 24.0) if n_a != 0 else 0.0
    inv["parity_b"] = parity(b)
    inv["parity_e"] = parity(e)
    inv["parity_a"] = parity(a)
    ...
    inv["kappa_a_over_e"] = kappa
    inv["sin_theta"] = sin_theta; inv["cos_theta"] = cos_theta
    inv["area_tri"] = area_tri; inv["r_in"] = r_in; inv["R_circ"] = R_circ
    inv["slender_CF"] = slender_CF; inv["lambda_rectum"] = lambda_rectum
    inv["semi_major"] = semi_major; inv["semi_minor"] = semi_minor
    inv["ellipse_area"] = ellipse_area
    inv["triangle_pyth_error"] = triangle_pyth_error
    inv["aspect_G_over_sumCF"] = aspect_G_over_sumCF
    ...
    nb = b / sum_abs_tuple; ne = e / sum_abs_tuple
    nd = d / sum_abs_tuple; na = a / sum_abs_tuple
    cos_angle_CG = safe_div(
        C * C + G * G - F * F, 2.0 * C * G,
    ) if (C != 0.0 and G != 0.0) else 0.0
    harmonic_density = (
        (abs(E_QA) + abs(L) + abs(H) + abs(I))
        / (1.0 + C * C + F * F + G * G)
    )
    inv["norm_triangle"] = norm_triangle
    inv["norm_ellipse"] = norm_ellipse
    inv["norm_mixed"] = norm_mixed
    inv["norm_b"] = nb; inv["norm_e"] = ne
    inv["norm_d"] = nd; inv["norm_a"] = na
    inv["cos_angle_CG"] = cos_angle_CG
    inv["harmonic_density"] = harmonic_density
...
def qa_feature_vector(
    b: float, e: float, mode: str = "qa21",
